...
## Оглавление
1. [Вводная информация](#1-вводная-информация)
2. [Подготовка проекта в Intellij IDEA](#2-подготовка-проекта-в-intellij-idea)
3. [Настройка Spring Security.](#3-настройка-spring-security)
4. [Запуск приложения.](#4-демонстрация-работы-приложения)
...
Свойство `spring.jpa.show-sql` позволяет выводить в консоль SQL-запросы, 
отправляемые к базе данных. Когда `spring.jpa.hibernate.ddl-auto` установлено 
в значение `update`, это означает, что таблицы и их поля в базе данных будут 
созданы или обновлены автоматически на основе сущностей (entity), определенных в
приложении. Таким образом, нам нужно лишь создать базу данных с именем `postgres`, 
а остальные таблицы (пользователи, роли и связующая таблица) с их внешними ключами будут сформированы автоматически по схеме, определенной в сущностях.
...
Аннотация `@EnableWebSecurity` в связке с **WebSecurityConfigurerAdapter** 
классом работает над обеспечением аутентификации и предоставляет пользовательские конфигурации.

**Spring boot** автоматически просканирует **WebSecurityConfig** и зарегистрирует его bean-компоненты благодаря аннотации `@Configuration`.
...
![](png/img5.png)
_Роль администратора можно поставить только через SQL запрос вручную_ 

# 5. Заключение.

В этом туториале мы рассмотрели, настройку Spring Security и выяснили
что это мощный и настраиваемый фреймворк аутентификации и контроля доступа для приложений 
на основе Spring. Он предоставляет широкий спектр функций по обеспечению 
безопасности приложений.

К сожалению у меня не получилось сместить фокус с настройки Spring Security и в этой статье
не были разобраны такие моменты, как создание сущностей, контроллеров, репозиториев и других
моментов необходимых для полного функционирования приложения. 

Но есть и хорошие новости! Полный код всего приложения можно на найти на моем [github](https://github.com/lesswixx/SpringSecutityTutorial)
